...
import { DesignStudioModal } from '~/components/DesignStudio/DesignStudioModal';
import {
  designOverlaysByView,
  frontAndBackGalleryImages,
} from '~/components/DesignStudio/designStudioLocations';
import { redirectIfHandleIsLocalized } from '~/lib/redirect';
import { isDecoratedProduct } from '~/lib/productFulfillment';
import {
  buildDesignLineAttributes,
  loadDesignSession,
...
export const meta = ({ data }) => {
  return [
    ...
    {
      rel: 'canonical',
      href: `/decorated-products/${data?.product.handle}`,
    },
  ];
};
...
async function loadCriticalData({ context, params, request }) {
  ...
  // Blank SKUs stay on `/products/:handle`.
  if (!isDecoratedProduct(product.tags)) {
    const url = new URL(request.url);
    const localeMatch = url.pathname.match(/^(\/[a-zA-Z]{2}-[a-zA-Z]{2})?\//);
    const localePrefix = localeMatch?.[1] || '';
    throw redirect(`${localePrefix}/products/${handle}${url.search}`);
  }
...
export default function Product() {
  ...
  const [savedDesign, setSavedDesign] = useState(
    /** @type {import('~/lib/designStudioApi').SavedProductDesign | null} */ (
      null
    ),
  );
  const [studioColors, setStudioColors] = useState(
    /** @type {Array<{code: string; name: string; image?: any; product?: any}>} */ (
      []
    ),
  );
  ...
  return (
    <>
                ...
                {(data) => {
                  ...
                  const allProducts = [
                    ...
                    ...data.products.nodes.filter(
                      (p) =>
                        isDecoratedProduct(p.tags) ===
                        isDecoratedProduct(product.tags),
                    ),
                  ];
      ...
      <DesignStudioModal
        ...
        colorName={
          studioColors.find((c) => {
            const a = String(c.code || '').replace(/^#/, '').toLowerCase();
            const b = String(selectedColor || '')
              .replace(/^#/, '')
              .toLowerCase();
            return a === b;
          })?.name || null
        ...
        onConfirmColor={(code) => {
          const match = studioColors.find((c) => {
            const a = String(c.code || '').replace(/^#/, '').toLowerCase();
            const b = String(code || '').replace(/^#/, '').toLowerCase();
            return a === b;
          });
          if (match?.product) {
            setSelectedColor(match.code.replace(/^#/, ''));
            setSelectedColorProduct(match.product);
            if (match.image) {
              colorImageCache.current.set(
                match.code.replace(/^#/, ''),
                match.image,
              );
            }
          } else {
            setSelectedColor(String(code || '').replace(/^#/, ''));
          }
        }}
      />
 ...
 *   onColorsReady?: (colors: Array<any>) => void;
 *   currentProductId: string;
 *   currentProduct: any;
 * }}
 */
    ...
    <Suspense fallback={null}>
      <Await resolve={relatedProducts}>
        {(data) => {
          ...
          const allProducts = data.products.nodes.filter(
            (p) =>
              isDecoratedProduct(p.tags) ===
              isDecoratedProduct(currentProduct.tags),
          );
...
/**
 * Push color list to parent once (for Design Studio) without setState-during-render.
 * @param {{ colors: Array<any>; onColorsReady?: (colors: Array<any>) => void }}
 */
function DesignStudioColorsBridge({colors, onColorsReady}) {
...
/** @typedef {import('./+types/decorated-products.$handle').Route} Route */
